...
public class LabelView extends LinearLayout {
    ...
    public void init(TagItem tagItem) {
        ...
        tagInfo.setClick(tagItem.getClick());
        if (tagItem.getType() == Constant.POST_TYPE_POI) {
    ...
    public void draw(ViewGroup parent, final int left, final int top, boolean isLeft) {
        ...
        if (parentWidth <= 0) {
            parentWidth = DemoApplication.getApp().getScreenWidth();
        }
    ...
    private int getImageWidth() {
        return imageWidth <= 0 ? DemoApplication.getApp().getScreenWidth() : imageWidth;
    }
